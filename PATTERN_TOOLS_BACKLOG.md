...
- **Stroke alignment (pattern mode).** A global Inside / Center (default) / Outside control (`actionChangePatternStrokeAlign`, shown only in pattern mode). Inside and outside place the full stroke width on the corresponding side of the on-grid path; center splits it evenly. Rendered as a **filled band** (`ShapeCache.generatePatternOffsetShape` / `getRenderShape` in `packages/element/src/shape.ts`) so thick miter corners remain exact. Render-only (outside the shared `ShapeCache`) so hit-testing/bounds use the true geometry. Offset primitive is `offsetPolyline` in `@excalidraw/math` (`packages/math/src/offset.ts`, unit-tested), winding-aware. Threaded through canvas, PNG, and SVG export via `renderConfig.patternStrokeAlign`; element-canvas cache regenerates on toggle like it does for `theme`. Stored in `appState.patternGridStrokeAlign`.
